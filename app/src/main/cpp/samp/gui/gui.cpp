...
bool UI::initialize()
{
    ...
    if(VER_x32)
    {
        d_label1 = new Label(cryptor::create("Client Beta | armeabi-v7a (x32)").decrypt(), ImColor(1.0f, 1.0f, 1.0f), true, UISettings::fontSize() / 3);
        this->addChild(d_label1);
        d_label1->setPosition(ImVec2(3.0, 3.0));
    }
    else
    {
        d_label1 = new Label(cryptor::create("Client Beta | arm64-v8a (x64)").decrypt(), ImColor(1.0f, 1.0f, 1.0f), true, UISettings::fontSize() / 3);
        this->addChild(d_label1);
        d_label1->setPosition(ImVec2(3.0, 3.0));
    }
...
void UI::renderDebug()
{
    ...
    char debugPools[20];
    snprintf(&debugPools[0], sizeof(debugPools), "Build Fix Lux Team");
